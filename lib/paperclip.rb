...
require_dependency "paperclip/uploaded_file"
...
module Paperclip
  ...
  #     config.stylists_path    = 'lib/pc'   # relative path to look for stylists, defaults to 'lib/paperclip_stylists'
  #   end
  #

  class << self
    def configure
      yield @config = Paperclip::Configuration.new
      config = @config
    end
    ...
    def config
      @config ||= Paperclip::Configuration.new
    end

    def require_stylists
      Dir.glob(File.expand_path("#{Paperclip.config.stylists_path}/*.rb")).sort.each do |stylist|
        #log('Requiring ' + stylist)
        require_dependency stylist
      end
    end
    ...
    def run_command cmd, params = "", expected_outcodes = 0
      command = %Q<#{%Q[#{path_for_command(cmd)} #{params}].gsub(/\s+/, " ")}>
      command = "#{command} 2>#{bit_bucket}" if Paperclip.options[:swallow_stderr]
      log("Executing command: #{command}") if Paperclip.options[:log_command]
    ...
    def stylist name #:nodoc:
      name = ActiveSupport::Inflector.classify(name.to_s)
      stylist = Paperclip.const_get(name)
      unless stylist.ancestors.include?(Paperclip::Stylist)
        raise PaperclipError.new("[paperclip] Stylist #{name} was not found")
      end
      stylist
    end
  ...
  module ClassMethods
    ...
    def has_attached_file name, options = {}
      ...
      property_options = options.delete_if { |k,v| ![ :public, :protected, :private, :accessor, :reader, :writer ].include?(k) }

      Paperclip::Resource::ATTRIBUTES.each do |a_name, a_data|
        property :"#{name}_#{a_name.to_s}", a_data[:type], property_options.merge(a_data[:options] || {})
      end
      ...
      define_method "#{name}?" do
        ! attachment_for(name).filename.blank?
      end
